...
def run_solution(dataset_train: torch.utils.data.Dataset, data_dir: str = os.curdir, output_dir: str = '/results/'):
    ...
    if not combined_model:
        ...
        # TODO General_1: Choose your approach here - chose the Baybes by Backdrop option
...
class MNISTNet(nn.Module):
    def __init__(self,
        ...
        self.layer6 = nn.Linear(100, out_features)
    ...
    def forward(self, x):
        ...
        x = F.dropout(
                F.max_pool1d(self.layer2(x), kernel_size = 5, stride = 1),
                p=self.dropout_p,
                training=self.training or self.dropout_at_eval
        )
        x = F.dropout(
                F.conv1d(self.layer3(x)),
                p=self.dropout_p,
                training=self.training or self.dropout_at_eval
        )
        x = F.dropout(
        ...
        x = F.dropout(
                F.relu(self.layer5(x)),
                p=self.dropout_p,
                training=self.training or self.dropout_at_eval
        )
        

        class_probs = self.layer6(x)
        return class_probs
...
class BayesianLayer(nn.Module):
    ...
    def __init__(self, in_features: int, out_features: int, bias: bool = True):
        ...
        #Prior distribution: What distribution the weight follow before
        #Blundell: Suggests a scale mixture of 2 Gaussians
        # P(w) = SUM (over w_j):{e*Gaussian(w_j;0,s_1)+ (1-e)*Gaussian(w_j;0,s_2)}
        #with s_1 > s_2
        ...
        if self.use_bias:
         ...
         #We have for every output cell a bias, 
         #herefore we create a row of Gaussian distribution whose size is equal to out_features
    ...
    def forward(self, inputs: torch.Tensor):
        ...
        # T: Perform a forward pass as described in this method's docstring.
